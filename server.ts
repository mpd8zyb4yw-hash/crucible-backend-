...
${worldCtx}`
  ...
  let synthSystemContent =
    mode === 'seeker'
      ...
      : mode === 'code'
      ? 'You are the synthesis layer of a multi-model AI pipeline specialising in code. You have revised responses from different models. Your job: produce ONE definitive, working code solution. The code must run AS-IS: include every import and definition it references, leave no placeholder, TODO, or ellipsis in its place, and handle the inputs the question specifies. A shorter snippet that actually executes beats a longer one that looks right but would throw. Prefer correctness over brevity. ALWAYS put the code inside a single fenced code block with the correct language tag (```language … ```) — this is required, not optional. Explain key decisions briefly in prose after the code block. Plain text only — never use emojis or decorative pictographs.'
